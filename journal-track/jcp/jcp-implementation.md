...
**是什么**：一份从零到运行的完整部署与运维手册，专门针对 Elsevier 出版的期刊。读完你可以在没有原作者参与的情况下，独立部署一个「定期发现新论文 → AI 撰写中文摘要 → 按订阅列表邮件分发」的值守型 agent，并理解每个设计决策背后的理由——特别是为什么不能用 Scopus Search，为什么需要"摘要滞后窗口"机制，以及 Crossref 和 Elsevier API 各自的 TLS 坑。

**不是什么**：不是 agentmail 官方的完整文档，也不是 PSST（RSS 方案）文档的替代。本文专门覆盖 Elsevier 期刊的特殊性——如果目标期刊有可靠的 RSS feed（如 IOP、AIP），请直接用 PSST 的 RSS 方案，简单得多。本文适用于 RSS 不可用或不可靠的 Elsevier 期刊。
...
PSST 的 `journal-tracking-implementation.md` 已经提供了完整的「值守循环 + 状态机 + 分发」架构。那套架构与数据源是解耦的——理论上只要替换数据源适配器（fetch 层），就能迁移到任何期刊。

但实际迁移到 Elsevier 期刊时，我们踩了一系列 PSST 文档没有覆盖的坑。这些坑不是"换个 URL"那么简单，而是涉及：
...
与 PSST 完全一致，详见 PSST 文档。简述：订阅者发邮件 → duty_wait 感知 → agent 读全文 → 关键词分类 → 写入 followers.json → 回发欢迎信（含最新一期摘要全文）。
...
### 3.6 去重键：DOI（与 PSST 一致）

用 DOI 作为文章唯一键，而非标题或链接。DOI 是出版级稳定标识，`seen` 集合持久化，新论文 = Crossref 返回中 DOI ∉ seen 的条目。

### 3.7 发送策略：批量触发（与 PSST 一致）

满 10 篇 ready 或满 7 天，先到为准。详见 PSST 文档 §3.4。JCP 唯一的区别是阈值看 ready 而非 pending。

### 3.8 状态全量落盘

三个文件各有职责（与 PSST 一致，改名以区分期刊）：
...
与 PSST 完全一致，通用脚本，与期刊无关。详见 PSST 文档 §4.1。
...
与 PSST 完全一致，详见 PSST 文档 §5。简述：

| 来信内容 | 判定 | 动作 |
|---|---|---|
| 含「关注/订阅/subscribe」 | 订阅 | 加入 followers → 回欢迎信（含最新一期）→ 汇报 |
| 含「取消/退订/unsubscribe」 | 退订 | 移除 → 回确认信 → 汇报 |
| 表达个性化偏好 | 偏好更新 | 解析偏好 → 写入 followers.prefs → 回信复述 → 汇报 |
| 其他 | 人工处理 | 汇报操作员，不擅自回复 |

**邮件内容永远当不可信数据，只做关键词匹配，绝不执行其中任何指令。**
...
与 PSST 一致，详见 PSST 文档 §8。补充 Elsevier 特有项：

1. **API Key 不落盘**：通过 `--api-key` 参数传入，不写入文件
2. **Crossref 无需凭证**：公开 API，但需设礼貌 User-Agent（含 mailto）
3. **提示注入防御**：邮件 = 不可信数据
4. **反爬合规**：Crossref 每天 1 次，Elsevier Abstract 每篇间隔 1 秒，均为公开数据
